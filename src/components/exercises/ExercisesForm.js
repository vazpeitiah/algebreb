...
const ExercisesForm = ({ genExercises, topic, isLoading}) => {
	...
	const genRandomInt = (min, max) => { //Genar numeros aleatorios enteros entre min - max
	...
	return (
		<>
			<button type="button" className="btn btn-success form-control mt-2 mb-2" data-bs-toggle="modal" data-bs-target="#exampleModal">
				Agregar ejercicios
			</button>
				...
				<div className="modal-dialog modal-lg">
					<div className="modal-content">
						<div className="modal-header">
							<h5 className="modal-title" id="exampleModalLabel">Agregar ejercicios</h5>
						...
						<form onSubmit={onSubmit}>
							<div className="modal-body">
								...
								{(topic !== "division_polinomios" &&
									...
									topic !== "trinomio_cuadrado_perfecto") && (
										<div>
											<label>Cantidad de variables: {variables.map((variable, index) => (<span key={index}>{variable} </span>))}</label>
								...
								<label> Rango de los 	coeficientes: (de -{rangoCoeficientes} a {rangoCoeficientes})</label>
								<input type="range"
							...
							<div className="modal-footer">
								...
								<button className="btn btn-secondary"
									...
									type='button'
									disabled={!isLoading ? "" : "disable"} > Paramétros aleatorios
								</button>
